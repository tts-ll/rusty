...
llvm_var* llvm_crate(GNode * crate){
...
llvm_var * llvm_maindef(GNode * maindef){
...
llvm_var * llvm_fndef(GNode * fndef){
	llvm_var * var = NULL;
	var = NULL;
	return var;
	...
	while(stmt){
		...
		switch( get_ast(stmt)->kind ){
			...
			case LETTYPE_STMT:
				...
				break;
			case PRINTI_STMT:
				var = llvm_printi(stmt->children->children);
				break;
			case PRINTS_STMT:
				var = llvm_prints(stmt->children->children);
				break;
			default:
				var = llvm_exp(stmt);
				break;
...
llvm_var * llvm_printi(GNode * exp){
/*
...
llvm_var * llvm_prints(GNode * exp){
	need_puts = 1;// will cause a declaration of puts for external linking
